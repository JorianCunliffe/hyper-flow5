...
PR [#56](https://github.com/JorianCunliffe/hyper-flow5/pull/56) merged the core implementation. Local test success does not establish a deployed capture tool or live provider acceptance. Use the [implementation guide](AMBIENT_WORK_CAPTURE_IMPLEMENTATION.md) and [API contract](API.md#captured-work-items) alongside this runbook.
...
5. Deploy the Communications `ambientCapture.js` adapter alongside HyperFlow. Existing `HYPERFLOW_AGENT_CONTEXT_URL`/`HYPERFLOW_EVENT_URL` and `COMMUNICATIONS_WEBHOOK_SECRET` settings are reused. Confirm the tenant primary Communications person maps to the intended Firebase primary user and that the phone line is configured. Inbound owner context enables the tool; outbound calls carry the trusted communication/thread identity. The tool calls signed `POST /api/agent/capture-work`, which independently validates membership and the source communication. Other providers require an equivalent adapter.
...
The Express Gemini `/api/live-voice` tool path is wired in code but still requires live verification. Vercel does not host its WebSocket. The Communications phone tool is registered in code. Authenticated desktop/mobile UI and an actual spoken-call capture remain live acceptance checks. Firebase rules and durable transactions passed the core PR CI.

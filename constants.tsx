...
import { GitBranch, RefreshCw, Mail, MessageSquare, Phone, Webhook, FileText, Circle, LucideIcon, BookOpen, TableProperties, BrainCircuit, Inbox, Clock3, Octagon, Radio } from 'lucide-react';
...
export const NODE_TYPE_META: Record<NodeType, NodeTypeMeta> = {
  ...
  [NodeType.WAIT]: { label: 'Wait', icon: Clock3, color: '#d97706', ringClass: 'ring-amber-300', description: 'Durably pauses a flow until its configured hold resolves' },
  [NodeType.END]: { label: 'End', icon: Octagon, color: '#475569', ringClass: 'ring-slate-400', description: 'Explicitly terminates this branch of the flow' },
  [NodeType.EVENT_TRIGGER]: { label: 'Event', icon: Radio, color: '#0891b2', ringClass: 'ring-cyan-300', description: 'Starts a flow occurrence when a matching trusted event arrives' },
  [NodeType.EMAIL]: { label: 'Email', icon: Mail, color: '#3b82f6', ringClass: 'ring-blue-200', description: 'Sends an email via the Communications API' },
  ...
  [NodeType.GOOGLE_DOC]: { label: 'Read Doc', icon: BookOpen, color: '#4285f4', ringClass: 'ring-blue-200', description: 'Reads a granted Google Doc resource' },
  [NodeType.GOOGLE_SHEET_READ]: { label: 'Read Sheet', icon: TableProperties, color: '#0f9d58', ringClass: 'ring-emerald-200', description: 'Reads a granted Google Sheet range resource' },

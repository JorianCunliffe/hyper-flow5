...
- a Bland callback resolving a pending run, merging the analysis into project
  data, and a downstream decision branching on it — with no browser involved
- `"true"` coerced to a real boolean, so strict `equals` branches match

...
const EmailDetail: React.FC<{
  ...
  const responsePresentation = triageResponsePresentation(item, matchingJob);
  const recommendedAction = triageRecommendedAction(item);
          ...
          <div className="space-y-5">
            {responseCommunicationId ? (
              ...
              <div className={`rounded-xl p-4 ${triageResponseToneClass(responsePresentation.tone)}`}>
                <p className="font-bold">{responsePresentation.label}</p>
                <p className="mt-1 text-sm opacity-90">{responsePresentation.detail}</p>
              </div>
            )}
            {recommendedAction && <div className="rounded-xl border border-slate-200 bg-white p-4"><h3 className="text-xs font-black uppercase tracking-wide text-slate-400">Recommended next step</h3><p className="mt-2 text-sm leading-6 text-slate-700">{recommendedAction}</p></div>}
...
const GenericDetail: React.FC<{ selection: Exclude<TriageSelection, { kind: 'email' } | { kind: 'digest' }>; replayingId: string | null; onReplay: (jobId: string) => Promise<void> }> = ({ selection, replayingId, onReplay }) => {
  ...
  if (selection.kind === 'coaching') {
    ...
    const presentation = coachingPresentation(session);
    return <><div className="border-b border-slate-200 p-5"><div className="flex items-center gap-2"><h2 id="triage-detail-title" className="pr-10 text-xl font-black text-slate-900">Coaching session</h2><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${coachingToneClass(presentation.tone)}`}>{presentation.label}</span></div><p className="mt-2 text-sm text-slate-500">{formatDate(session.scheduledFor || session.createdAt)} · Project {session.projectId}</p></div><div className="flex-1 overflow-y-auto p-5"><div className="grid grid-cols-3 gap-2"><div className="rounded-xl bg-violet-50 p-3 text-center"><div className="text-xl font-black text-violet-800">{typeof session.confidence === 'number' ? `${Math.round(session.confidence * 100)}%` : '—'}</div><div className="text-xs text-violet-700">Confidence</div></div><div className="rounded-xl bg-slate-50 p-3 text-center"><div className="text-xl font-black text-slate-900">{session.attemptCount || 0}</div><div className="text-xs text-slate-500">Attempts</div></div><div className={`rounded-xl p-3 text-center ${session.sheetWrite ? 'bg-emerald-50' : 'bg-slate-50'}`}><div className={`text-sm font-black ${session.sheetWrite ? 'text-emerald-800' : 'text-slate-600'}`}>{session.sheetWrite ? 'Recorded' : 'Not recorded'}</div><div className={`mt-1 text-xs ${session.sheetWrite ? 'text-emerald-700' : 'text-slate-500'}`}>Tracker update</div></div></div><div className={`mt-4 rounded-xl border p-4 text-sm ${presentation.tone === 'red' ? 'border-red-200 bg-red-50 text-red-800' : presentation.tone === 'amber' ? 'border-amber-200 bg-amber-50 text-amber-800' : 'border-violet-200 bg-violet-50 text-violet-900'}`}><p className="font-bold">What happened</p><p className="mt-1 leading-6">{presentation.detail}</p><p className="mt-3 font-bold">Recommended next step</p><p className="mt-1 leading-6">{presentation.nextStep}</p></div><h3 className="mb-2 mt-6 text-xs font-black uppercase tracking-wide text-slate-400">Coaching outcome</h3><dl className="rounded-xl border border-slate-200 px-4"><DetailRow label="Summary" value={session.summary} /><DetailRow label="Progress" value={session.progress} /><DetailRow label="Blockers" value={session.blockers} /><DetailRow label="Commitments" value={session.commitments} /><DetailRow label="Next actions" value={session.nextActions} />{session.failureReason && <DetailRow label="Failure" value={<span className="text-red-700">{session.failureReason}</span>} />}{session.nextRetryAt && <DetailRow label="Next retry" value={`${formatDate(session.nextRetryAt)} · ${formatLabel(session.retryStatus)}`} />}</dl><h3 className="mb-2 mt-6 text-xs font-black uppercase tracking-wide text-slate-400">Source and tracker</h3><dl className="rounded-xl border border-slate-200 px-4"><DetailRow label="Coaching document read" value={formatDate(session.documentReadAt)} /><DetailRow label="Tracker read" value={formatDate(session.sheetReadAt)} /><DetailRow label="Sheet range" value={session.sheetRange} /><DetailRow label="Updated" value={formatDate(session.updatedAt)} />{session.sheetWrite && <DetailRow label="Tracker write record" value={<pre className="overflow-x-auto whitespace-pre-wrap text-xs">{JSON.stringify(session.sheetWrite, null, 2)}</pre>} />}</dl><details className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-3"><summary className="cursor-pointer text-xs font-bold text-slate-600">Technical references</summary><div className="mt-3 space-y-3"><Identifier label="Session" value={session.id} /><Identifier label="Project" value={session.projectId} /><Identifier label="Schedule" value={session.scheduleId} /><Identifier label="Schedule run" value={session.scheduleRunId} /><Identifier label="Communication" value={session.communicationId} /><Identifier label="Transcript" value={session.transcriptId} /><Identifier label="Document" value={session.documentId} /><Identifier label="Spreadsheet" value={session.spreadsheetId} /></div></details></div></>;
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end" role="dialog" aria-modal="true" aria-labelledby="triage-detail-title">
      ...
      <aside className="relative flex h-full w-full max-w-2xl flex-col bg-white shadow-2xl">
        ...
        {selection.kind === 'digest' && <><div className="border-b border-slate-200 p-5"><h2 id="triage-detail-title" className="pr-10 text-xl font-black text-slate-900">Daily digest</h2><p className="mt-2 text-sm text-slate-500">{formatDate(selection.digest.scheduledFor)} · {formatLabel(selection.digest.deliveryStatus)}</p></div><div className="flex-1 overflow-y-auto p-5"><div className="grid grid-cols-3 gap-2"><div className="rounded-xl bg-slate-50 p-3 text-center"><div className="text-xl font-black text-slate-900">{selection.digest.counts.total}</div><div className="text-xs text-slate-500">New</div></div><div className="rounded-xl bg-amber-50 p-3 text-center"><div className="text-xl font-black text-amber-800">{selection.digest.counts.outstanding}</div><div className="text-xs text-amber-700">Outstanding</div></div><div className="rounded-xl bg-indigo-50 p-3 text-center"><div className="text-xl font-black text-indigo-800">{selection.digest.counts.draftsPrepared}</div><div className="text-xs text-indigo-700">Current drafts</div></div></div><p className="mt-3 text-xs text-slate-500">Drafts are counted across the current outstanding set, not only the new messages in this digest.</p><details className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-3"><summary className="cursor-pointer text-xs font-bold text-slate-600">View generated digest summary</summary><p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-slate-700">{selection.digest.summary}</p></details>{selection.digest.deliveryError && <p className="mt-4 rounded-xl bg-red-50 p-3 text-sm text-red-700">{selection.digest.deliveryError}</p>}<h3 className="mb-2 mt-6 text-xs font-black uppercase tracking-wide text-slate-400">Included communications</h3><div className="space-y-2">{selection.digest.itemIds.map(id => { const item = items.find(candidate => candidate.id === id); if (!item) return <div key={id} className="rounded-xl border border-slate-200 p-3 font-mono text-xs text-slate-500">{id}</div>; const itemResponse = triageResponsePresentation(item, agentJobs.find(job => job.communicationId === item.communicationId)); const itemAction = triageRecommendedAction(item); return <button key={id} type="button" onClick={() => onSelectEmail(item)} className="flex w-full items-start justify-between gap-3 rounded-xl border border-slate-200 p-3 text-left hover:border-indigo-300 hover:bg-indigo-50/30"><span className="min-w-0 flex-1"><span className="block truncate text-sm font-bold text-slate-800">{item.subject || item.sender || 'Communication'}</span><span className="mt-1 block text-xs text-slate-500">{item.sender} · {formatDate(item.occurredAt)}</span>{itemAction && <span className="mt-2 block line-clamp-2 text-xs leading-5 text-slate-600"><b>Action:</b> {itemAction}</span>}</span><span className="flex shrink-0 items-center gap-2"><span className={`rounded-full px-2.5 py-1 text-[11px] font-bold ${triageResponseToneClass(itemResponse.tone)}`}>{itemResponse.label}</span><Link2 className="text-slate-400" size={15} /></span></button>; })}{!selection.digest.itemIds.length && <p className="rounded-xl border border-dashed border-slate-300 p-5 text-center text-sm text-slate-500">No individual communications were included.</p>}</div></div></>}

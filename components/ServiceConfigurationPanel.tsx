...
import { AlertTriangle, CheckCircle2, Pause, Play, RefreshCw, Settings } from 'lucide-react';
import type { Project, TenantSchedule } from '../types';
import { firebaseService } from '../services/firebaseService';

export const ServiceConfigurationPanel: React.FC<{ project: Project; onConfigure: () => void }> = ({ project, onConfigure }) => {
  ...
  return <div className="mb-5 rounded-2xl border border-violet-200 bg-violet-50/60 p-4">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-black text-violet-950">Service Configuration</h3><p className="text-xs text-violet-700">{project.projectData?.project_template === 'daily_coaching' ? 'Daily Coaching' : 'Email Triage'} · {schedule?.enabled ? 'active' : 'paused or incomplete'}</p></div><div className="flex flex-wrap gap-2"><button onClick={onConfigure} className="flex items-center gap-1 rounded-lg border border-violet-300 bg-white px-3 py-1.5 text-xs font-bold text-violet-800"><Settings size={14} /> Edit / validate</button>{schedule && <><button disabled={working} onClick={() => void action('run')} className="flex items-center gap-1 rounded-lg bg-violet-700 px-3 py-1.5 text-xs font-bold text-white"><Play size={14} /> Run now</button><button disabled={working} onClick={() => void action('toggle')} className="flex items-center gap-1 rounded-lg border border-violet-300 bg-white px-3 py-1.5 text-xs font-bold text-violet-800">{schedule.enabled ? <Pause size={14} /> : <Play size={14} />}{schedule.enabled ? 'Pause' : 'Resume'}</button></>}</div></div>
    {error ? <p className="mt-3 flex items-center gap-1 text-xs font-bold text-red-700"><AlertTriangle size={14} />{error}</p> : !status ? <p className="mt-3 flex items-center gap-1 text-xs text-violet-700"><RefreshCw className="animate-spin" size={14} />Loading service health…</p> : <div className="mt-3 grid grid-cols-1 gap-2 text-xs text-violet-900 md:grid-cols-4"><div><b>Connection:</b> {project.projectData?.triage_connection_id || project.projectData?.coaching_workspace_connection_id || 'missing'}</div><div><b>Previous:</b> {status.lastRun ? `${status.lastRun.status} · ${status.lastRun.processedCount ?? 0}` : 'none'}</div><div><b>Next:</b> {schedule ? new Date(schedule.nextRunAt).toLocaleString() : 'not scheduled'}</div><div className="flex items-center gap-1">{status.scheduler?.overdue ? <AlertTriangle size={14} className="text-amber-600" /> : <CheckCircle2 size={14} className="text-emerald-600" />}<b>Tick:</b> {status.scheduler?.lastTickAt ? new Date(status.scheduler.lastTickAt).toLocaleString() : 'never'}</div></div>}
